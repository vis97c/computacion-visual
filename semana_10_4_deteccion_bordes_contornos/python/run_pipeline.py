...
def run_pipeline():
    ...
    print(f"Starting Taller Detección de Bordes y Contornos execution pipeline in: {base_dir}")
    
    # Use the local virtual environment's python if available, otherwise fallback to system python
    venv_python = os.path.join(base_dir, "venv", "Scripts", "python.exe")
    if not os.path.exists(venv_python):
        venv_python = sys.executable
        print(f"Venv Python not found at {venv_python}. Using current python interpreter: {venv_python}")
    else:
        print(f"Using Virtual Environment Python: {venv_python}")
        
    # 1. Run generate_test_images.py
    print("\n--- Step 1: Generating Synthetic Test Images ---")
    gen_script = os.path.join(base_dir, "generate_test_images.py")
    subprocess.run([venv_python, gen_script], check=True)
    
    # 2. Run edge_contour_taller.ipynb cells programmatically to save all 5 notebook plots
    print("\n--- Step 2: Executing Jupyter Notebook Cells and Exporting Plots ---")
    notebook_runner = os.path.join(base_dir, "run_notebook_cells.py")
    
    # Let's write the helper notebook runner dynamically if it doesn't exist
    if not os.path.exists(notebook_runner):
        with open(notebook_runner, "w", encoding="utf-8") as f:
            f.write('''import cv2
import numpy as np
...
''')
        print("Written dynamic 'run_notebook_cells.py' successfully.")
        
    subprocess.run([venv_python, notebook_runner], check=True)
    
    # 3. Run inspeccion_calidad.py
    print("\n--- Step 3: Running Standalone Quality Control Inspection ---")
    inspec_script = os.path.join(base_dir, "inspeccion_calidad.py")
    subprocess.run([venv_python, inspec_script], check=True)
    
    # 4. Run medicion_objetos.py
    print("\n--- Step 4: Running Standalone Object Measurement & Corner Detection ---")
    measure_script = os.path.join(base_dir, "medicion_objetos.py")
    subprocess.run([venv_python, measure_script], check=True)
